<section class="mb-20">
    ...
    <div class="grid grid-cols-[auto_1fr] items-center mb-8">
        ...
        <div>
            <span class="text-gray-500 text-sm uppercase tracking-wider font-medium">Fresh & New</span>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 lg:gap-8">
        @foreach($latestProducts as $product)
            <div
                ...
                <!-- New Badge -->
                <div class="absolute top-3 left-3 z-10 bg-primary text-white text-xs font-bold px-2 py-1 rounded">
                    NEW
                </div>
                ...
                <figure class="relative overflow-hidden h-56">
                    ...
                    @else
                        <div
                            class="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 grid grid-flow-col auto-cols-max gap-3 place-content-center transition-opacity duration-300">
                            ...
                            @auth
                                <form action="{{ route('cart.add', $product) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                            class="btn btn-circle btn-sm bg-white text-black hover:bg-success hover:text-white border-none" {{ $product->current_stock <= 0 ? 'disabled' : '' }}>
                            ...
                            @else
                                <a href="{{ route('login') }}"
                                   class="btn btn-circle btn-sm bg-white text-black hover:bg-success hover:text-white border-none">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                         fill="currentColor" viewBox="0 0 16 16">
                                        <path
                                            d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
                ...
                <div class="card-body">
                    ...
                    <div class="card-actions grid grid-cols-1 sm:grid-cols-2 gap-2 mt-4">
